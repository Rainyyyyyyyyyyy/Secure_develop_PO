...
- `Обход папки внутри CryptoActionsAES`
...
0. Выводит путь, относительно которого будут отсчитываться относительные пути;
1. Запрос пути к директории (абсолютный путь или относительный);
2. Запрос режима работы:
	 - `.encrypt` - шифрование всех найденных файлов;
	 - `.decrypt` - дешифрование всех найденных файлов.
3. Запрос пароля;
4. Выполнение обработки в соответствии с выбранным режимом для каждого файла с выводом очередного обработанного.

Дополнительно в интерфейсе используется команда `.reset` для повторного ввода параметров.
Также введена команда `.exit` для выхода из программы.
...
## Обработка ошибок
Проект содержит набор пользовательских исключений для:
...
## Тестирование
Тест-кейсы перечислены в подпапке репозитория `Tests`
